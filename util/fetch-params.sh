...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/KFXParams"
    else
        PARAMS_DIR="$HOME/.knoxfs-params"
    fi
fi
...
Failed to fetch the KFX zkSNARK parameters!
...
This script will fetch the KFX zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common KFX zkSNARK parameters. Note that it is
