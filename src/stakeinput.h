...
#ifndef PAWS_STAKEINPUT_H
#define PAWS_STAKEINPUT_H
...
public:
    ...
    virtual bool IsZPAWS() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
    virtual bool ContextCheck(int nHeight, uint32_t nTime) = 0;
};


class CPawsStake : public CStakeInput
{
private:
...
public:
    CPawsStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CPawsStake* NewPawsStake(const CTxIn& txin);
    ...
    bool IsZPAWS() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //PAWS_STAKEINPUT_H
