...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("PAWSMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("PAWSMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("PAWSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("paws-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake && (chainActive.Height() >= 403750)) { // New version can only produce blocks after this height
        ...
        LogPrintf("Running PAWSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("PAWSMiner exception");
    } catch (...) {
        LogPrintf("PAWSMiner exception");
    }

    LogPrintf("PAWSMiner exiting\n");
}
