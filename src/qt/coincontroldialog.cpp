...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select PAWS Outputs to Spend" :
            "Select Shielded PAWS to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::PAWS;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show PAWS address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
