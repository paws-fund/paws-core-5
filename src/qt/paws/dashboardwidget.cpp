...
#include "qt/paws/dashboardwidget.h"
#include "qt/paws/forms/ui_dashboardwidget.h"
#include "qt/paws/sendconfirmdialog.h"
#include "qt/paws/txrow.h"
...
DashboardWidget::DashboardWidget(PAWSGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarePaws, "square-chart-paws");
    //setCssProperty(ui->labelSquarezPaws, "square-chart-zpaws");
    setCssProperty(ui->labelPaws, "text-chart-paws");
    //setCssProperty(ui->labelZpaws, "text-chart-zpaws");
    ...
    setCssProperty(ui->labelAmountPaws, "text-stake-paws-disable");
    //setCssProperty(ui->labelAmountZpaws, "text-stake-zpaws-disable");
    ui->labelAmountMNRewards->setText("0 PAWS");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &PAWSGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif

    if (hasCharts) {
        ui->labelEmptyChart->setText(tr("You have no rewards yet"));
    } else {
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("PAWS")
    updateDisplayUnit();
}
...
const QMap<int, QMap<QString, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        //bool isPaws = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZPAWS;
        bool isPaws = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::StakeMint;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair PAWS, zPAWS
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            //paws = (pair.first != 0) ? pair.first / 100000000 : 0;
            //zpaws = (pair.second != 0) ? pair.second / 100000000 : 0;
            //chartData->totalPaws += pair.first;
            //chartData->totalZpaws += pair.second;
            QMap<QString, qint64> pair = chartData->amountsByCache[num];
            paws = (pair["paws"] != 0) ? pair["paws"] / 100000000 : 0;
            //zpaws = (pair["zpaws"] != 0) ? pair["zpaws"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalPaws += pair["paws"];
            //chartData->totalZpaws += pair["totalZpaws"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPaws.append(paws);    
        chartData->valuesMNRewards.append(mnrewards); 
        //chartData->valueszPaws.append(zpaws);

        int max = std::max(paws, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(106,179,221));
    set1->setColor(QColor(41,134,204));
    ...
    set0->append(chartData->valuesPaws);
    //set1->append(chartData->valueszPaws);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    //if (chartData->totalPaws > 0 || chartData->totalZpaws > 0) {
    if (chartData->totalPaws > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountPaws, "text-stake-paws");
        //setCssProperty(ui->labelAmountZpaws, "text-stake-zpaws");
    ...
    //forceUpdateStyle({ui->labelAmountPaws, ui->labelAmountZpaws});
    forceUpdateStyle({ui->labelAmountPaws, ui->labelAmountMNRewards});
    ui->labelAmountPaws->setText(GUIUtil::formatBalance(chartData->totalPaws, nDisplayUnit));
    //ui->labelAmountZpaws->setText(GUIUtil::formatBalance(chartData->totalZpaws, nDisplayUnit, true));
