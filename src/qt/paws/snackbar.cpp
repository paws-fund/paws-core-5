...
#include "qt/paws/forms/ui_snackbar.h"
#include "qt/paws/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(PAWSGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &PAWSGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
