...
#include "qt/paws/forms/ui_addresseswidget.h"
#include "qt/paws/addresslabelrow.h"
#include "qt/paws/addnewaddressdialog.h"
#include "qt/paws/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(PAWSGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto pawsAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(pawsAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(pawsAdd);
        if (walletModel->updateAddressBookLabels(pawsAdd, label.toUtf8().constData(),
