...
#include "qt/paws/send.h"
#include "qt/paws/forms/ui_send.h"
#include "qt/paws/addnewcontactdialog.h"
#include "qt/paws/qtutils.h"
#include "qt/paws/sendchangeaddressdialog.h"
#include "qt/paws/optionbutton.h"
#include "qt/paws/sendconfirmdialog.h"
#include "qt/paws/guitransactionsutils.h"
#include "qt/paws/loadingdialog.h"
...
SendWidget::SendWidget(PAWSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-paws");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onPAWSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onPAWSSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent PAWSs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield PAWSs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent PAWSs to shield."));
    }
}
...
void SendWidget::onPAWSSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto pawsAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(pawsAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(pawsAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(pawsAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
