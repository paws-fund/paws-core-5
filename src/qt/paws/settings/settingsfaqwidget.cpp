...
#include "qt/paws/settings/settingsfaqwidget.h"
#include "qt/paws/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/paws/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(PAWSGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendablePAWS,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendablePAWS,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendablePAWS,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("PAWS is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. PAWS incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("PAWS utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of PAWS to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendablePAWSContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received PAWS requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your PAWS wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendablePAWS->setText(unspendablePAWSContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of PAWS with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("paws-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.PAWS.org'>" + tr("#support in our Discord") + "</a>.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node PAWS core wallet with a "
               "requirement of 10,000 PAWS secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in PAWS Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the PAWS network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("10,000 PAWS per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 10,000 PAWS collateral "
    ...
    connect(ui->pushButton_UnspendablePAWS, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendablePAWS);});
    ...
    if (parent)
        connect(parent, &PAWSGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
