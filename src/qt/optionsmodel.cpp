...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PAWS);
    ...
    if (settings.contains("nAnonymizePawsAmount") || reset)
        gArgs.SoftSetArg("-anonymizepawsamount", settings.value("nAnonymizePawsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in paws.cpp during shotdown to also remove the window positions
