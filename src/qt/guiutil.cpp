...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZpaws)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZpaws)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZpaws);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZpaws)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZpaws)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZpaws);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter PAWS address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no PAWS: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::PAWS, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert paws:// to paws:
    //
    //    Cannot handle this later, because paws:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PAWS, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PAWS (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PAWS (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "PAWS.lnk";
}
...
{
    return GetAutostartDir() / "paws.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a paws.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=PAWS (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=PAWS (regtest)\n";
        else
            optionFile << "Name=PAWS\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the paws app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add paws app to startup item list
