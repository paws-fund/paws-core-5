...
#include "qt/paws/splash.h"
#include "qt/paws/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("paws-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in paws.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in paws.qrc)
...
/** Class encapsulating PAWS Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (PAWSGUI)
...
{
    window = new PAWSGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PAWSGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &PAWSGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(PAWSGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(PAWSGUI::DEFAULT_WALLET);
        ...
        // PAWS: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PAWSGUI::handlePaymentRequest);
        connect(window, &PAWSGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. PAWS can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(paws_locale);
    Q_INIT_RESOURCE(paws);
    ...
    /// 6. Determine availability of data directory and parse paws.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("PAWS Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("PAWS Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // paws: links repeatedly have their payment requests routed to this process:
