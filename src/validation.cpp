...
#include "zpaws/zerocoin.h"
#include "zpaws/zpawsmodule.h"
...
#if defined(NDEBUG)
#error "PAWS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in upaws) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_23_SAPLING_MAINTENANCE) && tx.IsShieldedTx())
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight > 1 && nHeight <= 4575031) {
        nSubsidy = 1 * COIN;
    }
	else if (nHeight > 4575031) {
        nSubsidy = 1 * COIN;
    }
    else {
        nSubsidy = 1 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	else if (nHeight > 403750) {
        ret = blockValue * 0.95;
    }
    else {
		ret = blockValue * 0.999;
	}
	return ret;
}
...
{
    util::ThreadRename("paws-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    bool fSaplingMaintenance =  (block.nTime > sporkManager.GetSporkValue(SPORK_23_SAPLING_MAINTENANCE));
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZPAWSModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Paws Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nPawsBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nPawsBadBlockBits)) {
            // accept PAWS block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= sporkManager.GetSporkValue(SPORK_20_SPAM_CHK))) {
    ...
    if (isPoS) {
        ...
        if (nHeight > DON_BLOCK) {
            ...
            CAmount charityFundPayment = CharityFund(blockValue);
            CAmount operationFundPayment = OperationFund(blockValue);

            int s = block.vtx[1]->vout.size();
            if (block.vtx[1]->vout[s - 1].scriptPubKey != ChFundScriptPubKey())
                return state.DoS(100, error("CheckBlock() : incorrect C fund payment"));
            if (block.vtx[1]->vout[s - 1].nValue != charityFundPayment)
                return state.DoS(100, error("CheckBlock() : incorrect C fund payment"));
            if (block.vtx[1]->vout[s - 2].scriptPubKey != OpFundScriptPubKey())
                return state.DoS(100, error("CheckBlock() : incorrect D fund payment"));
            if (block.vtx[1]->vout[s - 2].nValue != operationFundPayment)
                return state.DoS(100, error("CheckBlock() : incorrect D fund payment"));
          }
        ...
        // Inputs
        std::vector<CTxIn> pawsInputs;
        std::vector<CTxIn> zPAWSInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zPAWSInputs.push_back(stakeIn);
            }else{
                pawsInputs.push_back(stakeIn);
            }
        }
        const bool hasPAWSInputs = !pawsInputs.empty();
        const bool hasZPAWSInputs = !zPAWSInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                if(consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC)) {
                    ...
                    if (isPrivZerocoinSpend || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                            PublicCoinSpend publicSpend(params);
                            if (!ZPAWSModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
                                return false;
                            }
                            spend = publicSpend;
                        } else {
                ...
                if(hasPAWSInputs) {
                    ...
                    for (const CTxIn& pawsIn : pawsInputs)
                        if(pawsIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zPAWS inputs.
            if(hasZPAWSInputs) {
                for (const CTxIn& zPawsInput : zPAWSInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zPawsInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpaws");
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70005 (v5.3.3)
    // SPORK_15 will be used on 70006
...
CAmount CharityFund(const CAmount& amount)
{
    return (amount * sporkManager.GetSporkValue(SPORK_17_CHARITY_FUND)) / 100;
}

CAmount OperationFund(const CAmount& amount)
{
    return (amount * sporkManager.GetSporkValue(SPORK_18_OPERATION_FUND)) / 100;
}

CScript ChFundScriptPubKey()
{
	return CScript() << ParseHex("0482b1a8c7c35d29f2f2c21594aa1597f1dd54e649fad7bf842099c0af146528e03840d435ced4799f36342de52fe15a1359f25b1b4f7312a2e3258e9a62b8ded0") << OP_CHECKSIG;
}


CScript OpFundScriptPubKey()
{
	return CScript() << ParseHex("04df8eed52040dba88cd6033a373c1ee2fecbbd2b8177eb25670b894ebf7cf8379ad83a66e260c2f07354b27838cef0f38469760e4bf41c85de828e21885975b1d") << OP_CHECKSIG;
}
