...
CPawsStake* CPawsStake::NewPawsStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CPawsStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CPawsStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CPawsStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CPawsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CPawsStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CPawsStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CPawsStake::GetUniqueness() const
{
    //The unique identifier for a PAWS stake is the outpoint
...
const CBlockIndex* CPawsStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CPawsStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CPawsStake::ContextCheck(int nHeight, uint32_t nTime)
{
