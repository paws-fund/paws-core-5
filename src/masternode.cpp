...
bool CMasternode::IsInputAssociatedWithPubkey() const
{
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        for (const CTxOut& out : txVin->vout) {
            if (out.nValue == GetMstrNodCollateral(chainActive.Height()) && out.scriptPubKey == payee) return true;
        }
    }
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int nChainHeight, int& nDoS)
{
    ...
    // should be at least not earlier than block when 10000 PAWS tx got MASTERNODE_MIN_CONFIRMATIONS
