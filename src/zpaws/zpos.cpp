...
bool CLegacyZPawsStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZPawsStake from non zc-spend", __func__);
    ...
    *this = CLegacyZPawsStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zpaws stake origin", __func__);

    // All good
    return true;
}

CLegacyZPawsStake::CLegacyZPawsStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZPawsStake::GetIndexFrom() const
{
...
CAmount CLegacyZPawsStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZPawsStake::GetUniqueness() const
{
...
bool CLegacyZPawsStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zPAWS stake block: height %d outside range", __func__, nHeight);
