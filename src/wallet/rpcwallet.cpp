...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "\nReturn information about the given PAWS address.\n"
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new PAWS address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"pawsaddress\"    (string) The new paws address\n"
...
UniValue getnewstakingaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new PAWS cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(dest) || isStakingAddress)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PAWS address");

    const std::string strLabel = (request.params.size() > 1 ? request.params[1].get_str() : "");

    const CKeyID* keyID = boost::get<CKeyID>(&dest);
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from PAWS address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->HasAddressBook(*keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get PAWS address from addressBook");
...
UniValue listdelegators(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) PAWS address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new PAWS address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"pawsaddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"pawsaddress\"   (string, required) The paws address to be associated with a label.\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"pawsaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"pawsaddress\"  (string, required) The paws address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in PAWS to send. eg 0.1\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CTransactionRef& txNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || !isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PAWS staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PAWS spending address");
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"stakingaddress\"      (string, required) The paws staking address to delegate.\n"
            "2. \"amount\"              (numeric, required) The amount in PAWS to delegate for staking. eg 100\n"
            "3. \"owneraddress\"        (string, optional) The paws address corresponding to the key that will be able to spend the stake.\n"
...
UniValue viewshieldtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                ...
                "      \"address\" : \"pawsaddress\",     (string) The PAWS address involved in the transaction\n"
...
static SaplingOperation CreateShieldedTransaction(const JSONRPCRequest& request)
{
    ...
    if (!Params().GetConsensus().NetworkUpgradeActive(nextBlockHeight, Consensus::UPGRADE_V5_0) ||
            sporkManager.IsSporkActive(SPORK_23_SAPLING_MAINTENANCE)) {
...
UniValue shieldsendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "      \"amount\":amount    (numeric, required) The numeric amount in " + "PAWS" + " is the value\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"pawsaddress\",     (string) The paws address\n"
            "      amount,                 (numeric) The amount in PAWS\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signmessage \"pawsaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"pawsaddress\"  (string, required) The paws address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"pawsaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given pawsaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"pawsaddress\"  (string, required) The paws address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in PAWS received at this address.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in PAWS received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() > 4 ))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in PAWS received for this wallet.\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in PAWS received for this wallet in P2CS contracts.\n"
...
static UniValue legacy_sendmany(const UniValue& sendTo, int nMinDepth, std::string comment, bool fIncludeDelegated)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid PAWS address: ")+name_);
...
UniValue sendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shield PAWS addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The paws address (either transparent or shield) is the key,\n"
            "                                     the numeric amount in PAWS is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a PAWS address or hex-encoded public key.\n"
            "If 'label' is specified, assign address to that label.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of paws addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) paws address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"label\"      (string, optional) A label to assign the addresses to.\n"

            "\nResult:\n"
            "\"pawsaddress\"  (string) A paws address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in PAWS received by the address\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"pawsaddress\",    (string) The paws address of the transaction.\n"
            "    \"category\":\"category\",      (string) The transaction category (send|receive|orphan|immature|generate).\n"
            "    \"amount\": x.xxx,          (numeric) The amount in PAWS. This is negative for the 'send' category, and positive\n"
            "                                         for the 'receive' category,\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in PAWS. This is negative and only available for the \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"address\":\"pawsaddress\",    (string) The paws address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in PAWS. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in PAWS. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in PAWS\n"
            "  \"confirmations\" : n,     (numeric) The number of confirmations\n"
            "  \"bcconfirmations\" : n,             (numeric) DEPRECATED: Will be removed in a future release\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in PAWS\n"
            "      \"label\" : \"label\",              (string) A comment for the address/transaction, if any\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending PAWSs\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending PAWSs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"pawsaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; paws server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "1. minconf          (numeric, optional, default=1) The minimum confirmations to filter\n"
                "2. maxconf          (numeric, optional, default=9999999) The maximum confirmations to filter\n"
                "3. \"addresses\"    (string) A json array of paws addresses to filter\n"
                "    [\n"
                "      \"address\"   (string) paws address\n"
                "      ,...\n"
                "    ]\n"
                ...
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"address\" : \"address\",  (string) the paws address\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid PAWS address: ") + input.get_str());
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending PAWSs.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in PAWS/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total PAWS balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the PAWS balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the PAWS balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in PAWS\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in PAWS\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in PAWS\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in PAWS\n"
            ...
            "  \"paytxfee\": x.xxxx                       (numeric) the transaction fee configuration, set in PAWS/kB\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in paws.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in paws.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) PAWS value of the stakeable coins (minus reserve balance, if any)\n"
...
UniValue setstakesplitthreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 PAWS each)."
            + HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in PAWS).\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same PAWS address\n"
