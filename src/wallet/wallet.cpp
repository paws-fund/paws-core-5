...
/**
 * Fees smaller than this (in upaws) are considered zero fee (for transaction creation)
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++) {
        ...
        else if (fMasterNode && tx->vout[i].nValue == GetMstrNodCollateral(chainActive.Height())) {
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != GetMstrNodCollateral(chainActive.Height())) {
        strError = "Invalid collateral tx value, must be 10,000 PAWS";
        ...
        const bool fIncludeLocked) const
{
    ...
    // Check for only 1k utxo
    if (nCoinType == ONLY_1000 && output.nValue != GetMstrNodCollateral(chainActive.Height())) return res;
    ...
    // Skip locked utxo
    if (!fIncludeLocked && IsLockedCoin(wtxid, outIndex) && nCoinType != ONLY_1000) return res;
...
{
    const bool fIncludeColdStaking = false;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-paws-address
        ...
        std::vector<CStakeableOutput>* availableCoins)
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CPawsStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in PAWS) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of PAWS Core"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart PAWS Core to complete"));
            return nullptr;
        } else {
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_PAWS && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_PAWS);
        }
