...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight + 1, blockValue) - CharityFund(blockValue) - OperationFund(blockValue);
    CAmount nonPoSPayment = GetMasternodePayment(pindexPrev->nHeight + 1, blockValue);
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            unsigned int i = txNew.vout.size();
      			if (pindexPrev->nHeight + 1 >= DON_BLOCK) {
      				txNew.vout.resize(i + 3);
      				txNew.vout[i + 2].scriptPubKey = ChFundScriptPubKey();
      				txNew.vout[i + 2].nValue = charityFundPayment;
      				txNew.vout[i + 1].scriptPubKey = OpFundScriptPubKey();
      				txNew.vout[i + 1].nValue = operationFundPayment;
            ...
            //subtract mn payment from the stake reward
      			...
      			if (pindexPrev->nHeight >= DON_BLOCK) {
      				txNew.vout.resize(4);
      				txNew.vout[3].scriptPubKey = ChFundScriptPubKey();
      				txNew.vout[3].nValue = charityFundPayment;
      				txNew.vout[2].scriptPubKey = OpFundScriptPubKey();
      				txNew.vout[2].nValue = operationFundPayment;
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount requiredMasternodePayment = GetMasternodePayment(nBlockHeight, nReward, nMasternode_Drift_Count) - CharityFund(nReward) - OperationFund(nReward);
