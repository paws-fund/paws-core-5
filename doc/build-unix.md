UNIX BUILD NOTES
====================
Some notes on how to build PAWS Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile PAWS Core and the dependencies,
...
This will build paws-qt as well, if the dependencies are met.
...
memory available when compiling PAWS Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~paws/+archive/paws).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:paws/paws
    sudo apt-get update
...
To build PAWS Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build paws-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a paws-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip pawsd" to strip the debug
...
Security
--------
To help make your PAWS Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, PAWS Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, PAWS Core may be compiled in
