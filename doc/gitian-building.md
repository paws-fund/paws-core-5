...
*Setup instructions for a gitian build of PAWS Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the PAWS
...
to the PAWS GitHub release page.
...
- [Building PAWS Core](#building-paws-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/paws-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/paws-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/paws-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/paws-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/paws-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/paws-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/paws-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@paws.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/paws-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/paws-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/paws-osx-*-build.assert
```
