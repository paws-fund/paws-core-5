PAWS Core version *4.1.0* is now available from:  <https://github.com/paws-project/paws/releases>
...
Enabling major improvements over the keystore management, the PAWS wallet doesn't require regular backups as before, keys are following a deterministic creation path that can be verified at any time (before HD Wallet, every keypair was randomly created and added to the keypool, forcing the user to backup the wallet every certain amount of time or could end up loosing coins forever if the latest `wallet.dat` was not being used).
...
An extended description of this large work can be found in the PR [here](https://github.com/PAWS-Project/PAWS/pull/1327).
...
Dialogs can now be accepted with the `ENTER` (`RETURN`) key, and dismissed with the `ESC` key ([#1392](https://github.com/PAWS-Project/PAWS/pull/1392)).
...
Addresses in the dropdown of the "Send Transaction" and "Send Delegation" widgets are now automatically sorted by label with ascending order ([#1393](https://github.com/PAWS-Project/PAWS/pull/1393)).

### Custom Fee

The custom fee selected when sending a transaction is now saved in the wallet database and persisted across multiple sends and wallet's restarts ([#1406](https://github.com/PAWS-Project/PAWS/pull/1406)). The fee is now also validated against the maximum value (10000 times `minRelayTxFee`) and minimum value (`minTxFee`) ([#1576](https://github.com/PAWS-Project/PAWS/pull/1576)).

### Include delegations in send

The send and cold-staking page present a checkbox to make the automatic input selection algorithm include delegated (P2CS) utxos if needed. The option is unchecked by default. ([#1391](https://github.com/PAWS-Project/PAWS/pull/1391))

### Optional Hiding of Staking Charts

The staking charts can now be hidden at startup (with a flag `--hidecharts`) or at runtime with a checkbox in settings --> options --> display ([#1475](https://github.com/PAWS-Project/PAWS/pull/1475)).
...
Restore the previous locking state ("locked" or "locked for staking only") when the action is completed. ([#1387](https://github.com/PAWS-Project/PAWS/pull/1387))

### External Change Address Warning

A new warning/confirmation dialog is displayed if a custom change address is not part of the wallet. ([#1527](https://github.com/PAWS-Project/PAWS/pull/1527))

### Cold Stake delegations marked in Coin Control

The Coin Control window now includes an icon next to the select checkbox when the UTXO is a Cold Stake delegation. This shares the space with the locked UTXO indicator icon, and locked UTXO's take priority in their icon display. ([#1470](https://github.com/PAWS-Project/PAWS/pull/1470))

### Hide zPAWS balance info as needed

When the wallet contains no zPAWS, the zPAWS balance details will be hidden, reducing visual clutter.
...
```
"Confirmed","Date","Type","Label","Address","Amount (PAWS)","ID"
```
...
### zPAWS Backup Removed

Automatic zPAWS backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the paws.conf file or as startup flags):
- `autozpivbackup`
- `backupzpiv`
- `zpivbackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 PAWS, down  to 500 PAWS.
...
- The `-reservebalance` configuration/startup option has been removed ([#1373](https://github.com/PAWS-Project/PAWS/pull/1373)).


Dependencies
------------

The minimum required version of QT has been increased from 5.0 to 5.5.1 (the [depends system](https://github.com/paws-project/paws/blob/master/depends/README.md) provides 5.9.7)
...
### GUI
 ...
 - #1306 `65ba128634` [GUI] Hide privacy widget when the wallet has no zPAWS balance (furszy)
 - #1316 `bcb04a44a1` [Trivial] fix typo "recomended" in sendcustomfee dialog (random-zebra)
 - #1323 `e2c07184f9` [Qt] Hide zPAWS balances when they are zero (Fuzzbawls)
...
### Core Systems
 ...
 - #1308 `ca912fc823` [zPAWS] Public coin, a super for-each removed (furszy)
...
### Cleanup
 ...
 - #1290 `2ceeb2cca0` [zPAWS][Cleanup] Zerocoin Cleanup 1: remove Accumulators values (random-zebra)
 - #1291 `16d7dac5f7` [zPAWS][Cleanup] Zerocoin Cleanup 2: remove CZPivStake class (random-zebra)
 - #1293 `28e0048b3e` [zPAWS][Cleanup] Zerocoin Cleanup 3: remove old ZK proofs (random-zebra)
 - #1314 `185194bd7e` [zPAWS][Cleanup] Zerocoin Cleanup 4: further wallet cleaning (random-zebra)
 - #1322 `5bd387e18d` [zPAWS][Cleanup] Zerocoin Cleanup 5: further main.cpp cleaning (random-zebra)
 - #1326 `6bbd575860` [zPAWS][Cleanup] Zerocoin Cleanup 6: Remove zerocoin mint checks (random-zebra)
