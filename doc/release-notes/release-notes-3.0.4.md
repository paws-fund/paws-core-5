PAWS Core version 3.0.4 is now available from:
...
PAWS Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zPAWS protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
- #327 `aa1235a` [Main] Don't limit zPAWS spends from getting into the mempool (Fuzzbawls)
...
### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending zPAWS (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when pawsd --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] PAWS not responding on user quitting app (Aaron Langford)
