PAWS Core version *v5.0.0* is now available from:  <https://github.com/paws-project/paws/releases>
...
PAWS Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, PAWS Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run PAWS v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/paws` folder to the `%APPDATA%\PAWSParams` directory.
...
#### Overview
Users will be able to protect their financial information by sending and receiving PAWSs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://paws.org .
...
* Top bar showing shield PAWS balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield PAWS balance.
...
See the release notes from [v4.2.0](https://github.com/PAWS-Project/PAWS/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed PAWS supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in PAWS
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in PAWS
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zPAWSsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield PAWS)
  The `moneysupply` JSON return entry is the total PAWS supply (the sum of all transparent and shield PAWS)
...
| `getaccountaddress` | Returned the current PAWS address associated with an internal account | Account system removal |
| `setaccount` | Associate a PAWS address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given PAWS address | Account system removal |
| `getaddressesbyaccount` | Returned a list of PAWS addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a PAWS address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a PAWS address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zPAWS balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zPAWS mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zPAWS mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zPAWS amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zPAWS to a PAWS address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zPAWS mints to a PAWS address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zPAWSs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzpivseed` | Set the wallet's deterministic zPAWS seed to a specific value | Zerocoin deprecation |
| `getzpivseed` | Return the current deterministic zPAWS seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zPAWS seed | Zerocoin deprecation |
| `dzpivstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzpiv` | Do a search for deterministically generated zPAWS that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_paws unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake pawsd and unit tests: fixing missing MacOS framework. (furszy)
