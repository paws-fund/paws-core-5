PAWS Core version *4.3.0* is now available from:  <https://github.com/paws-project/paws/releases>
...
- The chainstate database (which is used for tracking UTXOs) has been changed from a per-transaction model to a per-output model ([See PR 1801](https://github.com/PAWS-Project/PAWS/pull/1801)). Advantages of this model are that it:
...
- LevelDB has been upgraded to version 1.22 ([See PR 1738](https://github.com/PAWS-Project/PAWS/pull/1738)). This version contains hardware acceleration for CRC on architectures supporting SSE 4.2. As a result, synchronization and block validation are now faster.
...
### Miner/Block Generation
 ...
 - #1809 `959d707bc9` [Miner] decouple zPAWS duplicated serials checks from CreateNewBlock (furszy)
