...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/PAWSParams"
    else
        PARAMS_DIR="$HOME/.paws-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
PAWS - install-params.sh

This script will install the PAWS zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common PAWS zkSNARK parameters. Note that it is
