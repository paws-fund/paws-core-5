...
class ReorgStakeTest(PawsTestFramework):
    ...
    def check_money_supply(self, expected_paws):
        # verify that nodes have the expected PAWS supply
        paws_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(paws_supply, [DecimalAmt(expected_paws)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that PAWS supply was properly updated after the reorgs
        self.log.info("Check PAWS supply...")
